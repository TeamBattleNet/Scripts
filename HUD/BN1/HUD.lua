...
local function display_edit_slots()
    if hud.game.in_folder() then
        for i=1,7 do
            gui.pixelText(104, 20+16*i, string.format("%2i", hud.game.get_cursor_offset_folder()+i));
        end
    elseif hud.game.in_pack() then
        for i=1,7 do
            gui.pixelText(  4, 20+16*i, string.format("%3i", hud.game.get_cursor_offset_pack()+i));
        end
    end
end
