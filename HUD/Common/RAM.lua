...
local main_rng_table;
local lazy_rng_table;
...
function ram.set.lazy_RNG_index(new_RNG_index)
    local new_RNG_value = ram.get.lazy_RNG_value_at(new_RNG_index);
...
function ram.set.main_RNG_index(new_RNG_index)
    local new_RNG_value = ram.get.main_RNG_value_at(new_RNG_index);
    if new_RNG_value then
...
function ram.simulate_shuffle_folder(seed)
    ...
    local regSlot = ram.get.reg_slot();
    local slot1, slot2;
    ...
    if regSlot == 255 then
        ...
        -- In BN6, place reg chip at top and move other chips down. (Works in BN4 as well).
        -- In other games, swap the reg slot with the top chip? Currently unsure of this.
